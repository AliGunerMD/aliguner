---
title: "TUS 2026 I.Dönem Sonuçları"
date: '2026-05-21T13:32:27+03:00'
subtitle: 'Branşlara göre MIN-MAX puanlar'
summary: 'TUS 2026 I.Dönem - Branşlara göre MIN-MAX puanlar'
authors: [admin]
slug: tus-2026-1
categories:
  - AcadeMD
tags:
  - OSYM
  - TUS
lastmod: '2026-05-21T13:32:27+03:00'
featured: yes
image:
  caption: ''
  focal_point: ''
...
links:
 ...
 - icon: github
   icon_pack: fab
   name: GitHub
   url: "https://github.com/AliGunerMD/aliguner/blob/main/content/post/2026-05-21-tus-26-1/index.en.Rmd"
 - icon: sistrix
   icon_pack: fab
   name: Outputs
   url: "https://github.com/AliGunerMD/aliguner/blob/main/content/post/2026-05-21-tus-26-1/Outputs_2026_1"
url_pdf: ""
url_code: ""
url_dataset: ""
url_project: ""
...
Branş-spesific sonuçları görmek için [Outputs](https://github.com/AliGunerMD/aliguner/blob/main/content/post/2026-05-21-tus-26-1/Outputs_2026_1) sayfasına;   
Merak edenler kodları görmek isterse de [GitHub](https://github.com/AliGunerMD/aliguner/blob/main/content/post/2026-05-21-tus-26-1/index.en.Rmd) sayfasına gidebilirsiniz.  
...
# 
kontenjan_tus <- pdftools::pdf_text(pdf = here::here("datasets", "kont_tablo07052026.pdf")) %>%
  as_tibble()
...
```{r}
## to TEST
clean_branch_2026 <- clean_tus_2026 %>%
    filter(
...
func_tus <- function(my_branch) {
  ...
  minmax_branch <- clean_branch_2026 %>%
    filter(min != 0) %>%
    summarise(
  ...
  doluluk_branch <- clean_branch_2026 %>%
    summarise(
  ...
  lowest <- clean_branch_2026 %>%
    filter(min != 0) %>%
    arrange(min) %>%
    slice_head() %>%
    pull(kurum_title)

  highest <- clean_branch_2026 %>%
    filter(min != 0) %>%
    arrange(desc(max)) %>%
    slice_head() %>%
    pull(kurum_title)
  ...
  branch_plot <- clean_branch_2026 %>%
    ...
    geom_point(aes(x = min, y = name), color = ifelse(clean_branch_2026$doluluk >= 100, my_greens[1], my_grays[1]), size = 2.5) +
    geom_point(aes(x = max, y = name), color = ifelse(clean_branch_2026$doluluk >= 100, my_greens[2], my_grays[2]), size = 3.5) +
    theme_light() +
    ...
    geom_text(aes(x = max, y = name, label = max),
      hjust = -.5, size = 3,
      color = ifelse(clean_branch_2026$doluluk >= 100, my_greens[2], my_grays[2])
    ) +
    geom_text(aes(x = equal_kucuk_puan, y = name, label = equal_kucuk_puan), hjust = 1.5, size = 2.5, color = ifelse(clean_branch_2026$doluluk >= 100, my_greens[1], my_grays[2]), check_overlap = TRUE) +
    labs(
      title = glue:::glue("TUS 2026/I, <span style='color: #95568F;'>{my_branch}</span>"),
...
# branch_names <- unique(clean_tus_2026$brans)

branch_names <- clean_tus_2026 %>%
...
output_dir <- "Outputs_2026_1"

# Klasör yoksa oluşturma kontrolü
if (!dir.exists(output_dir)) {
  dir.create(output_dir, recursive = TRUE)
}

# Grafik kaydetme işlemi
ggsave(branch_plot,
  file = file.path(output_dir, paste0(branch_name, ".jpg")),
  dpi = 300,
  width = 9,
  height = 14
)
...
scale_x_continuous(limits = c(minmax_branch_overall$min - 3, minmax_branch_overall$max + 2), breaks = seq(45, 85, 5)) +
 ...
 labs(
      title = "TUS 2026/I.Dönem Branşların Min-Max puanları ve Doluluk oranları<br>",
      subtitle = "",
      x = "\nPuan",
      y = "",
