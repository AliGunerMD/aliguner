---
title: "Surgery - Q1 Journal list"
date: '2022-04-11T16:13:27+03:00'
...
authors: [admin]
slug: surgery-q1-2022
categories:
  - AcadeMD
tags:
  - WOS
lastmod: '2022-04-11T16:13:27+03:00'
featured: no
image:
  caption: ''
  focal_point: ''
...
links:
 - icon: github
   icon_pack: fab
   name: GitHub
   url: "https://github.com/AliGunerMD/aliguner/blob/main/content/post/2022-04-11-surgery-q1-2022/index.en.Rmd"
 - icon: link
   icon_pack: fas
   name: Clarivate
...
Web of Science performance metrics were summarized in [another post](https://www.aliguner.com/post/impact-factor/).

I will only share some data that can be difficult to find for an academic, accompanied by visuals.

```{r}
imported_data <- read_csv(here("datasets", "wos_2022_surgery.csv"),
         skip = 2,
         na = c("n/a", "N/A", "")) %>% 
  clean_names() %>% 
  slice(1:(nrow(.)-3)) 
...
**Categories:** Surgery  
**JCR Year:** 2020  (Current most up-to-date list)  
...
# Featured figure
 ...
 figure_q1_sur <- table_2022_q1 %>% 
  ...
  geom_text_repel(
   data = subset(table_2022_q1, journal_name %in% gs_journals & x2020_jif > 10),
   nudge_x = -50,
   box.padding = 0.5,
   nudge_y = -.5,
   segment.size  = 0.2,
   ...
   max.iter = Inf) +
  geom_text_repel(
   data = subset(table_2022_q1, journal_name %in% gs_journals & x2020_jif > 5 & citable_items > 500),
   nudge_x = 350,
   box.padding = -0,
   nudge_y = .5,
   segment.size  = 0.2,
   ...
   direction = "x") +
  geom_text_repel(
   data = subset(table_2022_q1, journal_name %in% gs_journals & x2020_jif < 5 & x2020_jif > 4.5 & citable_items > 500),
   nudge_x = 150,
   box.padding = -0,
   nudge_y = -1.5,
   segment.size  = 0.2,
   ...
   direction = "x") +
  geom_text_repel(
   data = subset(table_2022_q1, journal_name %in% gs_journals & x2020_jif < 4.5 & x2020_jif > 4 & citable_items > 500),
   nudge_x = 150,
   box.padding = -0,
   nudge_y = -.5,
   segment.size  = 0.2,
   ...
   direction = "x") +
  geom_text_repel(
   data = subset(table_2022_q1, journal_name %in% gs_journals & x2020_jif < 10 & x2020_jif > 5 & citable_items < 500),
   nudge_x = 800,
   box.padding = 1,
   nudge_y = .78 * subset(table_2022_q1, journal_name %in% gs_journals & x2020_jif < 10 & x2020_jif > 5 & citable_items < 500)$x2020_jif,
   segment.size  = 0.2,
   segment.color = "#C0C0C0",
   direction = "y",
   max.iter = Inf) +
  geom_text_repel(
   ...
   nudge_x = 300,
   box.padding = .5,
   nudge_y = -.005 * subset(table_2022_q1, journal_name %in% gs_journals & x2020_jif < 5 & citable_items < 500)$x2020_jif,
   segment.size  = 0.2,
