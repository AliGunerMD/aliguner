---
title: "Surgery - Q1 Journal list (2023 list with 2022-IFs)"
date: '2023-09-01T22:13:27+03:00'
...
authors: [admin]
slug: surgery-q1-2022_23
categories:
  - AcadeMD
tags:
  - WOS
lastmod: '2023-09-21T16:13:27+03:00'
featured: no
image:
  caption: ''
  focal_point: ''
...
links:
 - icon: github
   icon_pack: fab
   name: GitHub
   url: "https://github.com/AliGunerMD/aliguner/blob/main/content/post/2023-09-20-surgery-q1-2022_23/index.en.Rmd"
 - icon: link
   icon_pack: fas
   name: Clarivate
...
```{r}
imported_data <- read_csv(here("datasets", "wos_2022_2023_surgery.csv"),
         skip = 2,
         na = c("n/a", "N/A", "")) %>% 
  clean_names() %>% 
  slice(1:(nrow(.)-3)) %>% 
  mutate(x2022_jif = as.numeric(str_remove_all(x2022_jif, "<")))
...
**Categories:** Surgery  
**JCR Year:** 2022  (Current most up-to-date list)  
...
clean_data <- imported_data %>% 
  ...
  mutate(journal_name = str_to_title(journal_name),
      ...
      mutate(across(everything(), ~str_replace_all(., ",", " "))) %>% 
  mutate_at(c("total_citations", "x2022_jif", "x2022_jci", "x5_year_jif", "jif_without_self_cites", "immediacy_index", "normalized_eigenfactor", "eigenfactor", "article_influence_score", "jif_percentile", "total_articles", "citable_items"), as.numeric) 
```
...
table_2023_q1 <- clean_data %>% 
  filter(jif_quartile == "Q1") %>% 
  select(journal_name, x2022_jif, jif_without_self_cites,  total_citations, total_articles, citable_items)
```


```{r}
table_q1_2023_surgery <- table_2023_q1 %>% 
  arrange(desc(x2022_jif)) %>% 
  flextable() %>%
  set_table_properties(width = 1, layout = "autofit") %>% 
  set_header_labels(i = 1, "journal_name" = "Journal name", "x2022_jif" = "Impact factor",  "jif_without_self_cites" = "IF without self-citation" , "total_citations" = "# of citation", "total_articles" =  "# of article", "citable_items" = "# of citable items" ) %>% 
 theme_booktabs() %>% 
 align(part = "all", align = "center", j=2:ncol(table_2023_q1)) %>% 
 add_header_lines(paste0('The list of ', nrow(table_2023_q1),' WOS-Q1 journals in the category "Surgery" \n(year = 2022, released in July 2023)')) %>% 
 fontsize(part = "header", size = 14, i = 1) %>% 
 align(part = "header", align = "center", i=1) %>% 
 align(part = "header", align = "center", i=2, j = 2:ncol(table_2023_q1)) %>% 
 bold(part = "header") %>% 
 ...
 hline(part="body", i = nrow(table_2023_q1)) %>% 
 add_footer_lines ("") %>% 
...
# Featured figure
 ...
 set.seed(2024)
 figure_q1_sur <- table_2023_q1 %>% 
  arrange(desc(x2022_jif)) %>% 
  ggplot(aes(citable_items, x2022_jif, label = journal_name)) +
  geom_point(aes(color = x2022_jif, size = total_citations)) +
  geom_text_repel(
   data = subset(table_2023_q1, journal_name %in% gs_journals & x2022_jif > 10),
   nudge_x = -50,
   box.padding = 0.5,
   nudge_y = -.5,
   segment.size  = 0.2,
   ...
   max.iter = Inf) +
  geom_text_repel(
   data = subset(table_2023_q1, journal_name %in% gs_journals & x2022_jif > 5 & citable_items > 500),
   nudge_x = 350,
   box.padding = -0,
   nudge_y = .5,
   segment.size  = 0.2,
   ...
   direction = "x") +
  geom_text_repel(
   data = subset(table_2023_q1, journal_name %in% gs_journals & x2022_jif < 5 & x2022_jif > 4.5 & citable_items > 500),
   nudge_x = 150,
   box.padding = -0,
   nudge_y = -1.5,
   segment.size  = 0.2,
   ...
   direction = "x") +
  geom_text_repel(
   data = subset(table_2023_q1, journal_name %in% gs_journals & x2022_jif < 4.5 & x2022_jif > 4 & citable_items > 500),
   nudge_x = 150,
   box.padding = -0,
   nudge_y = -.5,
   segment.size  = 0.2,
   ...
   direction = "x") +
  geom_text_repel(
   data = subset(table_2023_q1, journal_name %in% gs_journals & x2022_jif < 10 & x2022_jif > 5 & citable_items < 500),
   nudge_x = 800,
   box.padding = 1,
   nudge_y = .78 * subset(table_2023_q1, journal_name %in% gs_journals & x2022_jif < 10 & x2022_jif > 5 & citable_items < 500)$x2022_jif,
   segment.size  = 0.2,
   segment.color = "#C0C0C0",
   direction = "y",
   max.iter = Inf) +
  geom_text_repel(
   ...
   nudge_x = 300,
   box.padding = .5,
   nudge_y = -.005 * subset(table_2023_q1, journal_name %in% gs_journals & x2022_jif < 5 & citable_items < 500)$x2022_jif,
   segment.size  = 0.2,
